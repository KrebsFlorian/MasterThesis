...
/*
	...
	First all include statements are added, next the sycl queue is created, then the expressions to create the FIFO and actor objects and some relevant scheduler variables.
...
void Converter::create_main(Dataflow_Network *dpn, std::string path, program_options *opt) {
	...
	if (!opt->no_SYCL) {
		//add expression to create the sycl queue
		if (opt->target_CPU) {
			first_part.append("static cl::sycl::queue queue(cl::sycl::cpu_selector{});\n");
		}
		else {
			first_part.append("static cl::sycl::queue queue(cl::sycl::gpu_selector{});\n");
		}
	}
	...
	for (auto it = dpn->id_constructor_map.begin(); it != dpn->id_constructor_map.end(); ++it) {
		...
		if (actor_inf.OpenCL) {
			first_part.append("cl::sycl::event " + it->first + "_status = cl::sycl::event();\n");
			first_part.append("bool update_buffer_" + it->first + "{ false };\n");
		}
	}
	...
	for (auto it = actor_ids_sorted.begin(); it != actor_ids_sorted.end(); ++it) {
		...
		if (actor_inf.OpenCL) {
			...
			std::set<std::string> outgoing{ find_outgoing_ports(*it,dpn) };
			for (auto out = outgoing.begin(); out != outgoing.end(); ++out) {
				first_part.append("\t\t\t\t" + *out + ".sycl_write_done();\n");
			}
