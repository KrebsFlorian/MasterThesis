...
class Unit_Generator {
	...
	std::pair<std::string, std::string> convert_import(Token& t) {
		if (t.str == "import") {
			...
			//std::cout << "Path:" << path_to_import_file << std::endl;
			Unit_Generator gen(path_to_import_file, const_map, native_flag,path);
			auto output = gen.convert_unit_file(symbol, "\t");
			//std::cout << path_to_import_file << " Done\n";
			return output;
		}
	}

public:
	Unit_Generator(std::string _path, std::map<std::string, std::string>&map, bool& flag, std::string source_dir) :path{ source_dir }, token_producer { _path }, const_map(map), native_flag(flag),current_path(_path) {}
	...
	*/
	std::pair<std::string,std::string> convert_unit_file(const std::string requested_symbol,const std::string prefix = "") {
		...
		//std::cout << "Convertion of:" << current_path << std::endl;
		...
		while (next_token.str != "end") {
			...
			if (next_token.str == "@native") {
				//std::cout << "Native Function Deklaration\n";
