...
void Converter::convert_Actors(Dataflow_Network* dpn, program_options *opts, std::string native_header_include){
	...
	for (auto it = dpn->id_class_map.begin(); it != dpn->id_class_map.end(); ++it) {
		...
		dpn->id_constructor_map[it->first] = Actor_Generator{ dpn->actors_class_path[it->second],opts->FIFO_size,!(opts->no_SYCL),opts->target_CPU,opts->cores,dpn,it->first }.convert_RVC_to_Cpp(opts->source_directory,opts->target_directory,native_header_include);
	}
}


void Converter::convert_Actors(Dataflow_Network* dpn, program_options *opts, std::string native_header_include, CodeGeneratorGUI *GUI) {
