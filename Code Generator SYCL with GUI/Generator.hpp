...
class Actor_Generator {
	...
	//the string will be written in actor_name.hpp if SYCL is used
	std::string header_output;

	//the string will be written in actor_name.cpp if SYCL is used
	std::string source_output;

	//needed to insert the values for constant inputs, to minimize the constructor and avoid this reference in SYCL environments
	...
	//true: SYCL can be used to accelerate the execution of the actor, false: SYCl cannot be used
	bool OpenCL;

	//true if there are no guard conditions for the actions
	bool no_guards{ true };

	//true: map to CPU if SYCL is used, false: map to GPU
	bool CPU;
	...
	//number of cores - for the mapping of SYCL calls to the CPU
	int number_cores;
	...
	//Convertion is delayed until the final decision is made if SYCL can be used or not!!
	std::vector<Action_Buffer> buffered_actions{};

	//if a cyclic fsm is executed in sycl, the starting state of the cycle is stored here - the state where the sycl execution begins
	std::string start_state;

	//maps variable names containing values read from the fifos to expressions to read them from a sycl buffer
	std::map<std::string, std::string> FIFO_access_replace_map_for_sycl;
	...
	*/
	...
	/*
		...
		Additionally, a header is created containing the class definition with the FIFOs, variables and functions generated from actors as members and a source
		is created containing the implementations for the declarations in the header and constants and functions and procedures that were not generated from actions.
		The header and the source are only valid if the functions declared and defined in the header do not access class members (variables).
		Therefore, just use the header and the source if OpenCL is true, because in this case it can be guaranteed that there are no class variables.
	...
	*/
	std::string convert_actor(Token& t, std::string& imports, std::string& imports_function_declarations) {
		...
		//convert variables, constants, function, procedures and decide if SYCL can be used. Therefore actions are buffered until the rest is processed.
		while (t.str != "end") {
			if (t.str == "int" || t.str == "uint" || t.str == "String" || t.str == "bool" || t.str == "half" || t.str == "float") {
				...
				if (tmp.find("{\n") != std::string::npos) { //{\n in the string indicates list comprehension! this must go to the constructor
					...
					if (const_expr) {
						source_output.append(declaration);
					}
					else {
						header_output.append(declaration);
					}
				}
				else {
					if (const_expr) {
						source_output.append(tmp);
					}
					else {
				...
				std::string dekl = tmp.substr(0, end_of_dekl) + ";\n";
				source_output.insert(0, dekl);
			}
	...
	*/
	...
	/*
		...
		Returns the generated code as a string and inserts the header for it to the header_output class var and the implementation to the source_output class var.
	*/
	std::string convert_Port_declarations(FIFO_Expression_Buffer& token_producer) {
		...
		header_output.append("//Input FIFOs:\n");
		//convert input fifos
		while (t.str != "==>") {
			std::string type{ convert_type(t, token_producer,symbol_definition_map_for_actor) };
			...
			header_output.append("\t" + fifo_template_type + " " + name + ";\n");
		...
		header_output.append("//Output FIFOs:\n");
	...
	*/
	...
	/*
		Convert the buffered actions either with the use of SYCL if the OpenCL flag is true or without SYCL if OpenCL is false.
	...
	*/
	std::string convert_actions() {
		std::string output;
		//map method name to (Action Information, SYCL code for this action) - SYCL code is need to generate an action for the whole FSM cycle if possible
		...
		if (OpenCL && !no_guards && guards_parallelizable) {
			generate_SYCL_code_with_guards("\t");
			return output;
		}
		for (auto it = buffered_actions.begin(); it != buffered_actions.end(); ++it) {
			...
			if (cycleFSM && OpenCL && no_guards) {
				...
				if (act_inf.method_name != "initialize$") {//initialize = no sycl, because only fired once
					it->reset_buffer(); //rest the buffer to the start to convert it to SYCL to generate an action for the whole FSM cycle
					std::string sycl_string;
					if (CPU) {
						sycl_string = convert_action_cyclic_fsm_sycl(*it, "\t\t\t\t\t");
					}
					else {
						sycl_string = convert_action_cyclic_fsm_sycl(*it, "\t\t\t\t");
					}
					name_to_info_map[act_inf.method_name] = std::make_pair(act_inf, sycl_string);
				}
			}
			else if (OpenCL && no_guards) { //pure SYCL without cyclic fsm
				output += convert_action_sycl(*it, act_inf, "\t");
			}
			else {// not OpenCL or SYCL!
				output += convert_action(*it, act_inf, "\t");
				//std::cout << act_inf.method_name << std::endl;
			}
		}
		...
		else if (OpenCL && !no_guards && !guards_parallelizable) {//in this case no SYCL code has been generated despite the OpenCL flag an be true, therefore the flag is adjusted to this here
			OpenCL = false;
		}
	...
	*/
	...
	/*
		...
		The created function is inserted to the header_output and source_output. Thus, the function returns void/nothing.

	*/
	void generate_SYCL_code_with_guards(std::string prefix = "") {
		...
		for (auto it = buffered_actions.begin(); it != buffered_actions.end(); ++it) {
			...
			auto r = convert_action_to_sycl_code_lambda(*it, act_inf, prefix_for_actions);
		...
		//create function declaration for the header and definition to the source
		header_output.append("\tcl::sycl::event action_"+actor_name+"(cl::sycl::queue& queue);\n");
		source_output.append("\tcl::sycl::event " + actor_name + "::action_" + actor_name + "(cl::sycl::queue& queue) {\n");
		...
		for (auto it = act_inf.input_fifos.begin(); it != act_inf.input_fifos.end(); ++it) {
			...
			buffer_access.append("\t\t\tauto " + it->first + "$ptr = (*" + it->first + ".get_read_buffer(number$instances * ");
			buffer_access.append(std::to_string(it->second) + ")).get_access<cl::sycl::access::mode::read>($cgh); \n");
			if (CPU) {
				indices.append("\t\t\t\t\tint " + it->first + "$index =  $laufvar_3 * " + std::to_string(it->second) + ";\n");
			}
			else {
				indices.append("\t\t\t\tint " + it->first + "$index = $it.get_linear_id() * " + std::to_string(it->second) + ";\n");
			}
		}
		for (auto it = act_inf.output_fifos.begin(); it != act_inf.output_fifos.end(); ++it) {
			if (!first_calc_add) {
			...
			buffer_access.append("\t\t\tauto " + it->first + "$ptr = (*" + it->first + ".get_write_buffer(number$instances *");
			buffer_access.append(std::to_string(it->second) + ")).get_access<cl::sycl::access::mode::write>($cgh);\n");
			if (CPU) {
				indices.append("\t\t\t\t\tint " + it->first + "$index =  $laufvar_3 * " + std::to_string(it->second) + ";\n");
			}
			else {
		...
		source_output.append(calc_instances_expr + "});\n");
		if (CPU) {
			source_output.append(prefix + "\tint number$workitems = std::min( number$instances ," + std::to_string(number_cores) + ");\n");
		}
		//set up SYCL environment
		source_output.append(prefix + "\tauto $ev = $queue.submit([&](cl::sycl::handler& $cgh){\n");
		source_output.append(buffer_access);
		if (CPU) {
			source_output.append("\t\t\t$cgh.parallel_for<class " + actor_name + ">(cl::sycl::nd_range<1>(cl::sycl::range<1>(number$workitems),cl::sycl::range<1>(1)),[=](cl::sycl::nd_item<1> $it){\n");
			source_output.append("\t\t\t\tfor(int  $laufvar_3 = $it.get_global_linear_id();$laufvar_3 < number$instances; $laufvar_3 += number$workitems){\n");
		}
		else {
			source_output.append("\t\t\t$cgh.parallel_for<class " + actor_name + ">(cl::sycl::range<1>(number$instances),[=](cl::sycl::item<1> $it){\n");
		}
		source_output.append(indices);
		//place the sycl code in there
		for (auto it = actions.begin(); it != actions.end(); ++it) {
			std::string code = name_to_code_map[*it];
			if (it != actions.begin()) {
				code.insert(code.find("if"), "else ");
			}
			source_output.append(code);
		}
		//insert closing brackets and rest of the function
		if (CPU) {
			source_output.append("\t\t\t\t}\n");
		}
		source_output.append("\t\t\t});\n");
		source_output.append("\t\t});\n");
		source_output.append("\t\treturn ev;\n");
		source_output.append("\t}\n");
	}
	...
	*/
	std::pair<std::string,std::string> convert_action_to_sycl_code_lambda(Action_Buffer &token_producer,Action_Information& act_inf, std::string prefix = "") {
		Token t = token_producer.get_next_Token();
		std::map<std::string, std::string> local_map;
		std::string output{};
		...
		if (t.str == "guard") {
			...
			while (t.str != "var" && t.str != "do" && t.str != "end") {
				if (t.str == ",") {
					...
					if (FIFO_access_replace_map_for_sycl.find(t.str) != FIFO_access_replace_map_for_sycl.end()) {
						condition.append(FIFO_access_replace_map_for_sycl[t.str]);
						t = token_producer.get_next_Token();
	...
	/*
		...
		This function also appends the generated function declaration to the header_output and the complete function with class qualifiert to the
		source_output class variables.
	*/
	std::string build_FSM_Cycle_function(std::map<std::string, std::pair<Action_Information, std::string>>& map, std::string prefix) {
		...
		//create method code
		std::string output{ prefix + "cl::sycl::event FSM$Cycle$"+actor_name+"(cl::sycl::queue& $queue){\n" };
		//append function declaration to the header
		header_output.append(prefix + "cl::sycl::event FSM$Cycle$"+actor_name+"(cl::sycl::queue& $queue);\n");
		...
		//If it is mapped to the CPU there is additonally a loop that maps the number of instances to the number of cores
		std::string buffer_access;
		std::string indices;
		for (auto it = input_fifo_to_number_tokens.begin(); it != input_fifo_to_number_tokens.end(); ++it) {//pair<fifo_name,#tokens
			buffer_access.append(prefix + "\t\tauto " + it->first + "$ptr = (*" + it->first + ".get_read_buffer(number$instances*");
			buffer_access.append(std::to_string(it->second) + ")).get_access<cl::sycl::access::mode::read>($cgh);\n");
			...
			if (CPU) {
				indices.append(prefix + "\t\t\t\tint " + it->first + "$index =  $laufvar_3  * " + std::to_string(it->second) + ";\n");
			}
			else {
				indices.append(prefix + "\t\t\tint " + it->first + "$index = $it.get_linear_id * " + std::to_string(it->second) + ";\n");
			}
		}
		...
		for (auto it = output_fifo_to_number_tokens.begin(); it != output_fifo_to_number_tokens.end(); ++it) {
			buffer_access.append(prefix + "\t\tauto " + it->first + "$ptr = (*" + it->first + ".get_write_buffer(number$instances*");
			buffer_access.append(std::to_string(it->second) + ")).get_access<cl::sycl::access::mode::write>($cgh);\n");
			if (number_inst_condition.size() == 0) {
		...
		if (CPU) {
			output.append(prefix + "\tint number$workitems = std::min(" + std::to_string(number_cores) + ", number$instances);\n");
		}
		...
		for (auto state_it = states.begin(); state_it != states.end(); ++state_it) {
			...
			SYCL_Code.append(map[schedulable_actions.front()].second);
		}
		//generate SYCL environment
		output.append(prefix + "\tauto $ev = queue.submit([&](cl::sycl::handler& $cgh){\n");
		output.append(buffer_access);
		if (CPU) {
			output.append(prefix + "\t\t$cgh.parallel_for<class FSM_Cyle_" + actor_name + ">(cl::sycl::nd_range<1>(cl::sycl::range<1>(number$workitems),cl::sycl::range<1>(1)),[=](cl::sycl::nd_item<1> $it){\n");
			output.append(prefix + "\t\t\tfor(int  $laufvar_3 = $it.get_global_linear_id();$laufvar_3 < number$instances; $laufvar_3  += number$workitems){\n");
		}
		else {
			output.append(prefix + "\t\t$cgh.parallel_for<class FSM_Cyle_" + actor_name + ">(cl::sycl::range<1>(number$instances),[=](cl::sycl::item<1> $it){\n");
		}
		output.append(indices);
		//
		output.append(SYCL_Code);
		if (CPU) {
			output.append(prefix + "\t\t\t}\n"); //close for loop
		}
		output.append(prefix + "\t\t});\n");
		output.append(prefix + "\t});\n");
		output.append(prefix + "\treturn $ev;\n");
		output.append(prefix + "}\n");
		...
		//add actor_name:: for source
		std::string tmp{ output };
		tmp.insert(17, actor_name + "::");
		//append function with class qualifier to the source 
		source_output.append(tmp);
		return output;
	}
	...
	*/
	std::string convert_action_cyclic_fsm_sycl(Action_Buffer& token_producer, std::string prefix = "") { //returns the body of the inner sycl lambda
		std::map<std::string, std::string> local_map;
		Token t = token_producer.get_next_Token();
		Action_Information act_inf;
		std::string output;
	...
	/*
		...
		It converts the RVC code to C++ + SYCL framework code.
		...
		source_output class variables.
	*/
	std::string convert_action_sycl(Action_Buffer& token_producer, Action_Information& act_inf, std::string prefix = "") {
		std::map<std::string, std::string> local_symbol_map;
		std::string output{  };
		std::string end_of_output;
		std::string sycl_lambda;
		Token t = token_producer.get_next_Token();
		std::string action_tag;
		std::string action_name{ "" };
		std::string method_name;
		...
		if (t.str == "action") {//action has no name
			...
			token_producer.reset_buffer();
			return convert_action(token_producer, act_inf, prefix); //no need to convert it to a sycl action because it will be executed once anyhow
		}
		else {
			action_tag = t.str;
			t = token_producer.get_next_Token(); // : or .
		...
		output.append(prefix+"cl::sycl::event " + method_name + "(cl::sycl::queue& $queue){\n");
		//add function declaration to the header
		header_output.append(prefix+"cl::sycl::event " + method_name + "(cl::sycl::queue& $queue);\n");
		//continue conversion
		t = token_producer.get_next_Token();//start of the fifo part
		//input fifos
		while (t.str != "==>") {
			if (CPU) {
				sycl_lambda.append(convert_input_FIFO_access_sycl(t, token_producer, act_inf, local_symbol_map, prefix + "\t\t\t\t"));
			}
			else {
				sycl_lambda.append(convert_input_FIFO_access_sycl(t, token_producer, act_inf, local_symbol_map, prefix + "\t\t\t"));
			}
		...
		while (t.str != "do" && t.str != "guard" && t.str != "var" && t.str != "end") {
			if (CPU) {
				end_of_output.append(convert_output_FIFO_access_sycl(t, token_producer, act_inf, local_symbol_map, prefix + "\t\t\t\t"));
			}
			else {
				end_of_output.append(convert_output_FIFO_access_sycl(t, token_producer, act_inf, local_symbol_map, prefix + "\t\t\t"));
			}
		...
		//add scheduling conditions regarding the space of the fifos to conditions and create expression to calculate the number of instances and create the expression to get the sycl buffers from the FIFOs
		...
		//The FIFO accessors in SYCL are generated with fifo name.get_buffer.get_access
		//Additionally, every FIFO gets a index = sycl id * consumed or produced number of tokens per iteration
		//If its mapped to the CPU there is another additional loop to map the number of instances to the number of cores, this influences the fifo index generation!
		std::string calc_instances_expr{ prefix + "\tint number$instances = std::min({" };
		bool first_calc_add{ true };
		std::string buffer_access{};
		//create indices to access the buffers/accessors
		...
		if (CPU) {
			output.append(prefix + "\tint number$workitems = std::min( number$instances ," + std::to_string(number_cores) + ");\n");
		}
		//set up SYCL environment
		output.append(prefix + "\tauto $ev = $queue.submit([&](cl::sycl::handler& $cgh){\n");
		output.append(buffer_access);
		if (CPU) {
			output.append("\t\t\t$cgh.parallel_for<class " + method_name+"$"+actor_name + ">(cl::sycl::nd_range<1>(cl::sycl::range<1>(number$workitems),cl::sycl::range<1>(1)),[=](cl::sycl::nd_item<1> $it){\n");
			output.append("\t\t\t\tfor(int  $laufvar_3 = $it.get_global_linear_id();$laufvar_3 < number$instances; $laufvar_3 += number$workitems){\n");
		}
		else {
			output.append("\t\t\t$cgh.parallel_for<class " + method_name+"$"+actor_name + ">(cl::sycl::range<1>(number$instances),[=](cl::sycl::item<1> $it){\n");
		}
		output.append(indices);
		output.append(sycl_lambda);
		//read guards -> if the action is executed with sycl, there should be no guards!!!! Guards require a scheduling decision each time the action is fired - bad for sycl!
		/*if (t.str == "guard") {
		std::string guard;
		t = token_producer.get_next_Token();
		while (t.str != "var" && t.str != "do") {
		if (t.str == ",") {
		guard.append(" && ");
		}
		else if (t.str == "=") {
		...
		if (CPU) {
			output.append(prefix+"\t\t\t}\n"); // close for loop over all instances
		}
		output.append(prefix + "\t\t});\n");
		output.append(prefix + "\t});\n");
		if (condition.size() == 0) {
		...
		//add actor_name:: for source
		std::string tmp{ output + prefix + "\treturn $ev;\n" + prefix + "}\n" };
		tmp.insert(17, actor_name + "::");
		//add function with class qualifier to the source
		source_output.append(tmp);
		return output + prefix + "\treturn $ev;\n" + prefix + "}\n";
	}
	...
	/*
		...
		The result is returned as a string and is meant to be used only in SYCL kernels. Otherwise the way the FIFOs/Ports are accessed won't work.
	*/
	std::string convert_inline_if_with_list_assignment_sycl(Token& t, Token_Container& token_producer, std::map<std::string, std::string>& global_map, std::map<std::string, std::string>& local_map, std::string prefix, std::string fifo_name) {
		std::string condition;
		...
		if (nested) {//found another ...?...:... in the if body -> recursive function call of this part 
			...
			expression1 = convert_inline_if_with_list_assignment_sycl(tok_token,tok, global_map, local_map, prefix + "\t", fifo_name);
		}
		else {
			...
			if (expression1[0] == '[') {
				Tokenizer tok{ expression1,true };
				Token tok_token = tok.get_next_Token();
				expression1 = convert_list_comprehension_sycl(tok_token,tok, fifo_name, prefix + "\t",global_map,local_map);
			}
			else {
		...
		if (nested) { //found another ...?...:... in the else part -> recursive call
			...
			Token tok_token = tok.get_next_Token();
			expression2 = convert_inline_if_with_list_assignment_sycl(tok_token,tok, global_map, local_map, prefix + "\t", fifo_name);
		}
		else {
			//auf listenzuweiseung prüfen, wenn ja mit convert_list_comprehension in C++ Code umwandeln
			if (expression2[0] == '[') {
				Tokenizer tok{ expression2,true };
				Token tok_token = tok.get_next_Token();
				expression2 = convert_list_comprehension_sycl(tok_token,tok, fifo_name, prefix + "\t", global_map,local_map);
			}
			else {
	...
	The list comprehension is converted to a loop or multiple loops in C++ with SYCL.
	...
	*/
	std::string convert_list_comprehension_sycl(Token& t, Token_Container& token_producer, std::string fifo_name, std::string prefix, std::map<std::string, std::string>& global_map, std::map<std::string, std::string>& local_map) {
		std::string output;
		t = token_producer.get_next_Token();
	...
	The FIFO access is converted to C++ + SYCL framework code.
	...
	*/
	std::string convert_input_FIFO_access_sycl(Token& t, Action_Buffer& token_producer, Action_Information& act_inf, std::map<std::string, std::string>& local_map, std::string prefix = "") {
		...
		while (t.str != "==>") {
			...
			if (t.str == "repeat") {
				...
				int preview_index{ 0 };
				for (auto it = consumed_element_names.begin(); it != consumed_element_names.end(); ++it) {
					output.append(prefix + "auto " + *it + " = " + name + "$ptr[" + name + "$index++];\n");
					FIFO_access_replace_map_for_sycl[*it] = name + "$ptr[" + name + "$index + " + std::to_string(preview_index) + "]";
					++preview_index;
				}
				act_inf.input_fifos.push_back(std::make_pair(name, consumed_element_names.size()));
	...
	/*
		...
		The fifo accesses are converted to C++ + SYCL framework code and this code is returned as a string.
	...
	*/
	std::string convert_output_FIFO_access_sycl(Token& t, Action_Buffer& token_producer, Action_Information& act_inf, std::map<std::string, std::string>& local_map, std::string prefix = "") {
		...
		while (t.str != "do" && t.str != "guard" && t.str != "var" && t.str != "end") {
			...
			if (t.str == "[") {
				...
				output.append(convert_list_comprehension_sycl(t, token_producer, name, prefix,symbol_definition_map_for_actor,local_map));
				...
				while (t.str != "]") {
					std::string expr{ " " };
					bool add_to_expr_list{ true }; //conditional statements with list assignments shouldn't be added to the expr list, because the cpp code is generated elsewhere
					while (t.str != "," && t.str != "]") {
						if (t.str == "[") {
							while (t.str != "]") {
								expr.append(t.str);
							...
							if (tmp.second) {
								add_to_expr_list = false;
								Tokenizer tok{ tmp.first,true };
								Token tok_token = tok.get_next_Token();
								output.append(convert_inline_if_with_list_assignment_sycl(tok_token, tok, symbol_definition_map_for_actor, local_map, prefix, name));
								++expr_counter;
							}
							else {
	...
	//---------------------------- Pure Cpp without SYCL generation -----------------------------------
	...
	*/
	std::string convert_action(Action_Buffer& token_producer, Action_Information& act_inf, std::string prefix = "") {
		...
		//add function declaration to the header
		header_output.append(prefix + "void " + method_name + "();\n");
		//convert fifo access
		...
		//insert actor_name:: for source
		std::string tmp{ output + end_of_output + prefix + "}\n" };
		tmp.insert(6, actor_name + "::");
		//add function with class qualifier to the source
		source_output.append(tmp);
	...
	*/
	...
	/*
		...
		corresponding buffer to the token. Also the function takes the action information object for this action, the local map of this action and the prefix for each 
		line of code as arguments.
		The FIFO access is converted to pure C++ code, for SYCL environments there is another function.
		Every FIFO name with the number of consumed tokens is inserted into the action information object and every symbol declaration is inserted into the local map.
		The function returns the generated code in a string object.
	*/
	...
	/*
		The functions takes a token pointing at the first fifo name of the output part and the corresponding action buffer, the action information object
		for this action, the local map of this action and the prefix for each line of code as input parameters.
		The fifo accesses are converted to pure C++ code and this code is returned as a string. For the use in a sycl environment, there is another function to convert output fifo accesses.
		Additionally, each fifo name and number of consumed/produces tokens in this access are inserted into the output fifo list of the action information list.
		All defined symbols are inserted into the local map.
	*/
	...
	*/
	...
	/*
		...
		It distinguishes between three cases: OpenCL/SYCL with exactly one cycle in the FSM, OpenCL/SYCL and no OpenCL/SYCL.
		...
		In case of OpenCL/SYCL with exactly one cycle in the FSM, at the end the function that executes complete cycles is scheduled.

		This function returns the generated scheduler as a string.
		This function also appends the scheduler function declaration to the header_output class variable and the complete function with the class qualifier is appended to the source_output.
	*/
	std::string generate_scheduler() {
		std::string output{ "\t" };
		if (OpenCL) {
			output.append("cl::sycl::event schedule(cl::sycl::queue& queue, bool& update_fifo){\n");
			header_output.append("\tcl::sycl::event schedule(cl::sycl::queue& queue, bool& update_fifo);\n");
			if (cycleFSM) {
				...
				output.append(cond + " ) {\n");
				output.append("\t\t\tupdate_fifo = true;\n");
				output.append("\t\t\treturn FSM$Cycle$"+actor_name+"(queue);\n");
				output.append("\t\t}\n");
				//no state change here because a complete cycle is executed!!!
			}
			else {//OpenCL/SYCL but not cycleFSM
			...
			output.append("\t\treturn cl::sycl::event();\n");
		}
		else {//Pure C++
			output.append("void schedule(){\n\t\tfor(;;){\n");
			header_output.append("\tvoid schedule();\n");
			if (FSM) {
				for (auto it = states.begin(); it != states.end(); ++it) {
	...
	/*
		...
		This function also appends the constructor declaration to the header_output and the complete function with the class qualifier to the source_output class variables.
	*/
	std::string generate_constructor() {
		...
		header_output.append(output + ");\n");//add method signature/declaration to the header
...
public:
	/*
		Constructor:
			...
			3. parameter: boolean if SYCL shall be used where possible, if true SYCL shall be used, if false not.
			4. parameter: boolean indicating if SYCL calls shall be mapped to the CPU (true), or not (false)
			5. parameter: the number of cores the SYCL calls should be mapped to. For max. performance it should be equal or lower to the number of cores of the executing system
			6. parameter: a pointer to the Dataflow_Network object containg all the relevant data about the network
	*/
	Actor_Generator(std::string path_to_actor, int fifo_size, const bool realize_with_sycl, const bool CPU, const int cores, Dataflow_Network *_dpn, std::string _id) :token_producer{ path_to_actor }, FIFO_size{ fifo_size }, CPU{ CPU }, OpenCL{ realize_with_sycl }, number_cores{ cores }, dpn{ _dpn }, id{ _id } {
	...
	/*
		...
		Addition: Due to problems in the SYCL implementation, SYCL cannot dereference this references, thus in case of SYCL usages 
				  the code has to be split into a header and a source file and functions and const vars are no longer part of the class.
	*/
	Converted_Actor convert_RVC_to_Cpp(std::string path_to_src_directory, std::string path_storage, std::string native_include_statement) {
		...
		if (actor_name.size() != 0) {
			if (OpenCL) {//header and source to one file due to a bug? in SYCL
				std::ofstream header_file{ path_storage + "\\" + actor_name + ".hpp" };
				if (header_file.bad()) {
					throw Converter_Exception{ "Cannot open file " + path_storage + "\\" + actor_name + ".hpp" };
				}
				//wrap into a namespace to avoid name collisions with other header files, because it cannot be split up into header and implementation and not be in a class because sycl cannot dereference 'this'.
				header_file << "namespace " + actor_name + " {\n";
				header_file << header_output;
				source_output.insert(0, "//-------------------------------------------------------------------------------\n");
				header_file << source_output;
				header_file << "}\n";
			}
			else {
