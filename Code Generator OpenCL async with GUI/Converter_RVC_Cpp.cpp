...
namespace Converter_RVC_Cpp {
	...
	namespace {
		...
		std::string find_type_of_list(const std::string str) {
			...
			else if (str.find("true") != std::string::npos || str.find("false") != std::string::npos) {
				return "bool";
			}
			else {
				return "int";
		...
		std::string convert_list_comprehension(Token & t, Token_Container & token_producer, std::string list_name, std::map<std::string, std::string>& global_map, std::map<std::string, std::string>& local_map, std::string prefix, std::string outer_expression, bool nested);
	...
	namespace {
		...
		int get_size_of_list(const std::string str) {
			...
			while (t.str != "}" ) {
				if (t.str == "{" ) {//nested list comprehension
					...
					if (t.str == ":") {
						...
						int size_of_list = get_size_of_list(tmp_command);
						tmp_command.insert(0, "{");
						tmp_command.append("}");
						std::string unused_var{ find_unused_name(global_map,local_map) };
						output.append(prefix + find_type_of_list(tmp_command) + " " + unused_var + "["+std::to_string(size_of_list)+"] = " + tmp_command + ";\n");
						output.append(prefix + "for( int i = 0;i < "+ std::to_string(size_of_list)+"; i++) {\n");
						output.append(command+ " = " + unused_var + "[i];\n");
	...
	std::string convert_function(Token& t, Token_Container& token_producer, std::map<std::string, std::string>& global_map, std::string prefix, std::string symbol) {
		...
		if (t.str == "function") {
			...
			while (t.str != ")") {
				if (t.str == "uint" || t.str == "int" || t.str == "String" || t.str == "bool" || t.str == "half" || t.str == "float") {
					tmp.append(convert_type(t, token_producer, global_map, local_map));
			...
			tmp.append(")");
			t = token_producer.get_next_Token();
			if (t.str != "-->")std::cerr << "Error parsing function";
			t = token_producer.get_next_Token();//must be the return type
			output.append(convert_type(t, token_producer, global_map, local_map) +tmp +" {\n");
			t = token_producer.get_next_Token();
			while (t.str != "end") {
