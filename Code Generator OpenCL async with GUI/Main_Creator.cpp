...
void Converter::create_main(Dataflow_Network *dpn, std::string path, program_options *opt) {
	...
	for (auto it = dpn->id_constructor_map.begin(); it != dpn->id_constructor_map.end(); ++it) {
		...
		if (it->second.OpenCL) {
			first_part.append("cl_event *" + it->first + "_event = NULL;\n");
		}
	}
	//create the global scheduler
	first_part.append("void global_scheduler() {\n");
	if (!opt->no_OpenCL) {
		first_part.append("\tcl_int status_flag = CL_COMPLETE;\n");
	}
	...
	for (auto it = actor_ids_sorted.begin(); it != actor_ids_sorted.end(); ++it) {
		...
		if (!dpn->id_constructor_map[*it].OpenCL) {
			//check if it is running, if not than start 
			...
			first_part.append("\t\tif("+*it+"_event != NULL)clGetEventInfo(*"+*it+"_event,CL_EVENT_COMMAND_EXECUTION_STATUS, sizeof(cl_int),&status_flag,NULL);\n");
			first_part.append("\t\tif(status_flag == CL_COMPLETE && !" + *it + "_running.test_and_set() ){\n");
			first_part.append("\t\t\t" + *it + "_instance->schedule("+*it+"_event);\n");
			first_part.append("\t\t\t" + *it + "_running.clear();\n");
			first_part.append("\t\t}\n");
		}
